...
)
{
	...
	printf("wait_flag1111111---s32Ret = %ld====%1d====%1d====%1d====%1d\r\n", s32Ret,u08Adr,u08Mask,u08Val,u08TO);


	while ( s32Ret	== YAD1DD_SUCCESS )
	{
        printf("wait_flag222222222---s32Ret = %ld===== %1d\r\n", u08Page,u08Adr);
		s32Ret	= YAD1DD_Read( u08Page, u08Adr, &u08Reg, 1 );
        printf("wait_flag333333333---s32Ret = %ld===== %1d=====%1d\r\n", u08Reg,u08Mask,u08Val);
...
)
{
	...
	if (( pu08Script	!= NULL )
	 && ( u32Size		!= 0 ))
	{	
		printf("run_1_command>>>>>>>>pu08Script = %x,size = %ld\r\n", pu08Script[0], u32Size);
...
)
{
	 ...
	 && ( u32Size	!= 0 ))
	{
		...
		printf("YAD1DD_Read page=%x, com1=%x, com2=%x \r\n", u08Page,u08Com1, u08Com2);
		if (( s32Ret	== YAD1DD_SUCCESS )
		 && ( u08Com1	!= YAD1DD_SPI_COM_READ ))
		{
			s32Ret	= YAD1MD_Write( u08Com1, &u08Adr, 1 );
            		//printf("YAD1MD_Write3333     s32Ret= %x  ==== %x =====%x \r\n", u08Page,s32Ret,u08Com1);

		}
		if ( s32Ret	== YAD1DD_SUCCESS )
		{
			s32Ret	= YAD1MD_Read( u08Com2, pu08Data, u32Size );
                        	//	printf("YAD1MD_Read3333     s32Ret= %x  ==== %x =====%x \r\n", u08Com2,s32Ret,pu08Data);
...
)
{
	...
	s32Ret	= YAD1DD_ERROR_ARG;
	printf("YAD1DD_BatchProcess111111 s32Ret = %ld=====%d\r\n", pu08Script,u32Size);
